...
def _legacy_hard_stop_config(**overrides) -> dict:
    cfg = {
...
def test_legacy_hard_stop_config_does_not_block_repeated_exact_failure():
    agent = _make_agent("web_search", config=_legacy_hard_stop_config())
    args = {"query": "same"}
    _seed_exact_failures(agent, "web_search", args)
    starts = []
    progress = []
...
def test_legacy_same_tool_hard_stop_threshold_is_ignored_at_runtime():
    agent = _make_agent(
        "terminal",
        config=_legacy_hard_stop_config(
            hard_stop_after={
                "exact_failure": 99,
                "same_tool_failure": 2,
                "idempotent_no_progress": 5,
    ...
    tc = _mock_tool_call("terminal", json.dumps({"command": "bad-3"}), "c-legacy")
    msg = SimpleNamespace(content="", tool_calls=[tc])
    messages = []
    ...
    mock_hfc.assert_called_once()
    assert "same_tool_failure_warning" in messages[0]["content"]


def test_legacy_hard_stop_config_concurrent_path_executes_every_call_in_order():
    agent = _make_agent("web_search", config=_legacy_hard_stop_config())
    repeated_args = {"query": "repeated"}
    allowed_args = {"query": "allowed"}
    _seed_exact_failures(agent, "web_search", repeated_args)
    starts = []
    progress_events = []
    ...
    calls = [
        _mock_tool_call("web_search", json.dumps(repeated_args), "c-repeated"),
    ...
    assert starts == [
        ("c-repeated", "web_search", repeated_args),
    ...
    assert started_events == [
        ("tool.started", "web_search", repeated_args, {}),
        ("tool.started", "web_search", allowed_args, {}),
    ]
    assert len(completed_events) == 2
    assert all(event[1] == "web_search" for event in completed_events)


def test_plugin_pre_tool_block_wins_without_tool_execution():
    agent = _make_agent("web_search")
    args = {"query": "same"}
...
def test_legacy_hard_stop_config_run_conversation_warns_and_reaches_text_response():
    agent = _make_agent("web_search", max_iterations=10, config=_legacy_hard_stop_config())
    same_args = {"query": "same"}
    responses = [
        _mock_response(
            content="",
