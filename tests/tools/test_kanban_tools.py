...
def test_progress_publisher_skips_unsubscribed_task(monkeypatch, worker_env):
    ...
    try:
        assert [
            e for e in kb.list_events(conn, worker_env) if e.kind == "progress"
        ] == []
    finally:
        conn.close()
...
def test_progress_publisher_loop_repeats_each_interval_until_run_is_stale(monkeypatch):
    ...
    def publish(identity=None):
        identities.append(identity)
        return next(outcomes)

    monkeypatch.setattr(kt, "publish_current_worker_progress_once", publish)
    identity = ("task-1", 7)
    kt._current_worker_progress_publisher_loop(
        60.0,
        identity,
        ImmediateEvent(),
    )
    ...
    assert identities == [identity, identity, identity]
