...
class AIAgent:
    ...
    @staticmethod
    def _redact_interim_visible_text(text: str) -> str:
        """Redact user-visible progress fail-closed without logging its input."""
        try:
            return redact_visible_text(text)
        except Exception:
            logger.warning(
                "Codex interim commentary redaction failed; suppressing visible update"
            )
            return ""
    ...
    def _extract_codex_interim_visible_parts(
        ...
        for item in items:
            ...
            if visible:
                ...
                visible = self._redact_interim_visible_text(visible)
            if visible:
