...
Hermes records non-secret digests for each conversation-contract component. Tool descriptions and registry order may change without restarting the lineage, while tool names and parameter schemas still trigger a safe new root. If an unexplained contract change would create a third full bootstrap across consecutive ticks, Hermes pauses the job before the model call instead of continuing to spend. An explicit job update or successful compatible resume resets this guard, and normal resume remains reversible.

A finite, project-scoped persistent job also protects itself from completion loops. When `workdir` is set and the job returns the cron silence marker on two consecutive successful ticks **without calling any tools**, Hermes records both runs and pauses the job automatically. A silent tick that used tools does not count, so a job that checked real state is not stopped merely because there was nothing to report. Persistent jobs without `workdir` are not auto-paused by this guard. Explicit resume or trigger resets the counter, as do useful output, a tool-using silent tick, failure, interruption, a new root, or a session-mode change.
