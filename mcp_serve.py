...
def create_profile_router_mcp_server(
    *,
    ...
    The stdio developer path remains local and unauthenticated, so it exposes
    only the read-only default tool surface. The HTTP v1 path is self-hosted,
    localhost-bound by default, and protects additional private tools with
    hash-stored personal bearer tokens generated by Hermes.
    """
    ...
    assert_default_tools_are_no_model()
    assert_no_model_loop_tools_absent()
    router_tool_metadata = get_router_tool_metadata()
    stdio_public_tool_names = frozenset(
        name
        for name, metadata in router_tool_metadata.items()
        if metadata.get("enabled_by_default")
        and not metadata.get("mutates_state")
        and metadata.get("execution_status") == "executable_no_model"
    )
    ...
    def _call_tool(
        ...
        **kwargs,
        ...
        try:
            if not http_auth and tool_name not in stdio_public_tool_names:
                raise ProfileRouterAuthError(
                    "transport_not_authorized",
                    "Private profile-router tools require authenticated HTTP mode",
                )
            if http_auth:
    ...
    if not http_auth:
        # FastMCP 1.26 keeps the registered tools in this pinned manager map.
        # Filter after registration so private wrappers are absent from MCP
        # discovery, not merely rejected after a client has already selected
        # one. Fail closed if that internal contract changes in a future MCP
        # upgrade: exposing every private tool would be the unsafe fallback.
        tool_manager = getattr(mcp, "_tool_manager", None)
        registered_tools = getattr(tool_manager, "_tools", None)
        if not isinstance(registered_tools, dict):
            raise RuntimeError(
                "Cannot safely expose profile-router over stdio: "
                "FastMCP tool registry is unavailable"
            )
        for tool_name in tuple(registered_tools):
            if tool_name not in stdio_public_tool_names:
