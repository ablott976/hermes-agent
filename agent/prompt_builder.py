...
KANBAN_GUIDANCE = (
    ...
    "4. **Human progress.** Interim commentary may be delivered as a `Kanban "
    "update`. Keep it brief: finished, now, next, blocker. Never expose task "
    "ids, paths, commands, tools, models/providers, prompts, logs, traces, or "
    "other internals.\n"
    "5. **Block on genuine ambiguity.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess. "
    "The user will unblock with context and the dispatcher will respawn you.\n"
    "6. **Finish with the review model encoded by the task graph.** Always "
