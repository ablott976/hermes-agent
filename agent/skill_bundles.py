...
def build_bundle_invocation_message(
    ...
    ``None`` if the bundle wasn't found. With ``return_member_ids=True`` the
    lists contain configured member identifiers rather than editable
    frontmatter names; persistent runtime fingerprints use that stable shape.
