...
describe('BillingSettings', () => {
  ...
  it('rejects auto-refill amounts outside the billing bounds', async () => {
    ...
    expect(await screen.findByText(/^Threshold: minimum is .*10.*\.$/)).toBeTruthy()
  ...
  it('disables buy controls while polling', async () => {
    apiMocks.fetchBillingState.mockResolvedValue(okBilling(postTrainBillingState))
    apiMocks.fetchSubscriptionState.mockResolvedValue(okSubscription(postTrainSubscriptionState))
    ...
    apiMocks.chargeStatus.mockReturnValue(new Promise(() => {}))
  ...
  it('renders the settled charge outcome', async () => {
    apiMocks.fetchBillingState.mockResolvedValue(okBilling(postTrainBillingState))
    apiMocks.fetchSubscriptionState.mockResolvedValue(okSubscription(postTrainSubscriptionState))
    apiMocks.charge.mockResolvedValue({
      data: {
    ...
    apiMocks.chargeStatus.mockResolvedValue({
      data: {
        amount_usd: '25',
        ok: true,
        status: 'settled'
    ...
    expect(await screen.findByText(/^.*25.* added\. Balance is refreshing\.$/)).toBeTruthy()
  })
