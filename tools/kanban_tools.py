...
def set_current_worker_progress(text: str) -> bool:
    """Remember the latest safe human commentary for this worker process."""
...
def publish_current_worker_progress_once(
    expected_identity: Optional[tuple[str, int]] = None,
) -> str:
    """Append one deduplicated, run-scoped progress event."""
    ...
    if run_id is None:
        return "stale"
    if expected_identity is not None and (task_id, run_id) != expected_identity:
        return "stale"
    with _progress_state_lock:
...
def _current_worker_progress_publisher_loop(
    interval: float,
    identity: tuple[str, int],
    stop: threading.Event,
) -> None:
    global _progress_publisher_thread, _progress_publisher_identity
    global _progress_publisher_stop
    try:
        while not stop.wait(interval):
            outcome = publish_current_worker_progress_once(identity)
...
def ensure_current_worker_progress_publisher() -> bool:
    ...
    global _progress_publisher_stop
    with _progress_state_lock:
        # A constructed-but-not-yet-started thread is already owned; checking
        # only is_alive() leaves a double-start race between callbacks.
        if _progress_publisher_thread is not None:
            if _progress_publisher_identity == identity:
                return True
            if _progress_publisher_stop is not None:
                _progress_publisher_stop.set()
        stop = threading.Event()
        thread = threading.Thread(
            target=_current_worker_progress_publisher_loop,
            args=(interval, identity, stop),
            name=f"kanban-progress-{task_id[:12]}",
            daemon=True,
        )
        _progress_publisher_thread = thread
        _progress_publisher_identity = identity
        _progress_publisher_stop = stop
    try:
        thread.start()
    except Exception:
        with _progress_state_lock:
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    if not tid:
        return False
    # This activity edge is the earliest reliable point shared by all workers.
    ensure_current_worker_progress_publisher()
